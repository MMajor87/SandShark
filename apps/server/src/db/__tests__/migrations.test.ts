...
import { seedLegacyCalendar } from '../../__tests__/seed';
...
const REBUILD_MIGRATION = '0019_eminent_patch';
...
describe('migrations', () => {
  ...
  test('should snapshot the database before applying pending migrations', async () => {
    ...
    // stop before the upstream integration adds users.token_version
    await migrateDatabase(
      sqlite,
      db,
      buildPartialMigrationsFolder(REBUILD_MIGRATION)
    ...
    expect(created.at(-1)).toContain(`before-${REBUILD_MIGRATION}`);
  ...
  test('should not cascade away message relations when rebuilding a table', async () => {
    ...
    expect(count('SELECT COUNT(*) c FROM calendar_events')).toBe(1);
    expect(count('SELECT COUNT(*) c FROM calendar_event_invitees')).toBe(1);
    expect(
      sqlite.query('SELECT token_version FROM users WHERE id = 1').get()
